...
| :------------ | :------------------------------------------ | :-------------- | :----------------------------------------------------------- |
| `user`        | `aiload:user:{user_id}`                     | `HASH`          | 缓存用户的核心信息。字段名与数据库列名完全一致。             |
| `user_token`  | `aiload:user_token:{token}`                 | `STRING`        | 将 `token` 字符串映射到其对应的 `user_token_id` (`uint64`)。 |
|               | `aiload:user_token_details:{user_token_id}` | `HASH`          | 缓存用户令牌的详细信息，包括 `user_id` 和 `token_limit`。    |
| `platform`    | `aiload:platform:{platform_id}`             | `HASH`          | 缓存第三方 AI 平台的详细信息。                               |
| `model`       | `aiload:model:{model_id}`                   | `HASH`          | 缓存 AI 模型的详细信息，包括 `platform_id` 和 `name`。       |
| `model_alias` | `aiload:model_alias:{alias_name}`           | `STRING`        | 将模型别名映射到一个或多个 `model_id` (`uint64`)。           |
| `api_key`     | `aiload:api_key:{api_key_id}`               | `HASH`          | 缓存 API 密钥的详细信息，例如 `platform_id`。                |
